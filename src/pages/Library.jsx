...
export default function Library() {
  ...
  return (
    <div className="p-6 md:p-8 max-w-6xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Biblioteca Comercial</h1>
        <p className="text-gray-500 text-sm mt-1">Base de conhecimento que alimenta as respostas sugeridas pela IA</p>
      </div>
            ...
            {categories.map(cat => (
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors ${
                  selectedCat === cat.id ? 'bg-orange-50 text-orange-700 font-medium' : 'text-gray-600 hover:bg-gray-50'
                }`}
              >
                <span>{cat.icon || '📋'}</span>
                <span className="flex-1 text-left truncate">{cat.name}</span>
                <span className="text-xs text-gray-400">{entries.filter(e => e.category_id === cat.id).length}</span>
              </button>
            ))}
            {categories.length === 0 && <p className="text-sm text-gray-400 p-3">Nenhuma categoria</p>}
          </div>
        </div>
              ...
              <div className="flex items-center justify-between mb-4 gap-2">
                <div className="min-w-0">
                  ...
                  {selectedCategory.description && (
                    <p className="text-sm text-gray-500 truncate">{selectedCategory.description}</p>
                  )}
                </div>
                <div className="flex gap-2 flex-shrink-0">
                  ...
                  <Button size="sm" className="bg-orange-500 hover:bg-orange-600" onClick={() => { setEditingEntry(null); setShowEntryEditor(true); }}>
              ...
              <div className="space-y-3">
                {catEntries.length === 0 ? (
                  <div className="bg-white rounded-xl border border-gray-100 p-8 text-center">
                    ...
                    <p className="text-gray-400 text-sm">Nenhuma entrada nesta categoria ainda</p>
                  </div>
                ) : (
                    ...
                    <div key={entry.id} className="bg-white rounded-xl border border-gray-100 p-5">
                      <div className="flex items-start justify-between mb-3 gap-2">
                        ...
                        <div className="flex gap-1 flex-shrink-0">
                          <button onClick={() => { setEditingEntry(entry); setShowEntryEditor(true); }} className="p-1.5 text-gray-400 hover:text-gray-700">
                            <Edit2 className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => handleDeleteEntry(entry.id)} className="p-1.5 text-gray-400 hover:text-red-500">
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        </div>
                      </div>
                      <div className="bg-orange-50/50 rounded-lg p-3 mb-3">
                        <p className="text-xs text-orange-600 mb-1">Resposta recomendada</p>
                        <p className="text-sm text-gray-700 whitespace-pre-wrap">{entry.answer}</p>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                        {entry.objective && <div><p className="text-xs text-gray-400">Objetivo</p><p className="text-gray-600">{entry.objective}</p></div>}
                        {entry.technique && <div><p className="text-xs text-gray-400">Técnica</p><p className="text-gray-600">{entry.technique}</p></div>}
                        {entry.common_mistakes && <div><p className="text-xs text-gray-400">Erros comuns</p><p className="text-red-500">{entry.common_mistakes}</p></div>}
                        {entry.next_step && <div><p className="text-xs text-gray-400">Próximo passo</p><p className="text-orange-600">{entry.next_step}</p></div>}
                      </div>
                    </div>
                  ))
                )}
