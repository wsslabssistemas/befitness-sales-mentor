  ...
  return (
    <div className="bg-card rounded-2xl border border-border p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <p className="font-semibold text-foreground">Performance por Vendedor</p>
      </div>
      {list.length === 0 ? (
        <p className="text-center text-muted-foreground py-8 text-sm">Nenhum atendente registrou interações ainda</p>
      ) : (
        <div className="space-y-3">
          {list.map(s => {
            const rate = s.interactions > 0 ? ((s.matriculas / s.interactions) * 100).toFixed(0) : 0;
            return (
              <div key={s.name} className="flex items-center gap-4 p-3 rounded-xl bg-secondary group">
                {onDelete && (
                  <button
                    onClick={() => {
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 text-xs text-muted-foreground mt-1 flex-wrap">
                    ...
                    <span className="text-orange-400">{s.visitas} visitas</span>
                    <span className="text-purple-400">{s.experimentais} trials</span>
                    {s.overdue > 0 && <span className="text-red-400 font-medium">{s.overdue} atrasados</span>}
                    <span className="text-muted-foreground">{s.assigned} clientes</span>
                  </div>
                </div>
                <div className="text-right flex-shrink-0">
                  <div className="flex items-center gap-1.5 justify-end">
                    <Trophy className="w-4 h-4 text-orange-500" />
                    <span className="text-xl font-bold text-foreground">{s.matriculas}</span>
                  </div>
                  <p className="text-xs text-muted-foreground">{rate}% conversão</p>
                </div>
              </div>
            );
          })}
